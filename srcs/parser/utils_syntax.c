...
static	char	*remove_extra_spaces_in_line(char *var)
{
	...
	i = 0;
	j = 0;
	newstr = (char *)malloc(sizeof(char) * (ft_strlen(var) + 1));
	if (!newstr)
		return (NULL);
	while (var[i])
	{
...
static	char	*get_env_var_splited(char *str, t_all *all)
{
	...
	newstr = NULL;
	var = get_env_str(str, all);
	if (!var)
		return (NULL);
	trimmed_var = ft_strtrim(var, " \t");
	if ((all->params.quotes[1] % 2) || !trimmed_var)
		return (var);
	if (all->params.quotes[1] % 2 == 0)
	{
		newstr = remove_extra_spaces_in_line(trimmed_var);
		if (!newstr)
		{
			free(trimmed_var);
			free(var);
...
static	int	check_dollar(t_all *all, char **word, int start, int *arr)
{
	...
	free(*word);
	(*word) = ft_calloc(arr[1] - arr[0] + start + ft_strlen(tempory) + 1, \
				sizeof(char));
	if (!(*word))
		return (0);
	len = -1;
